DROP TRIGGER IF EXISTS `yuriichuk_7_72`.`catalog_tree_BEFORE_UPDATE`;

DELIMITER $$
USE `yuriichuk_7_72`$$
CREATE DEFINER=`root`@`localhost` TRIGGER `catalog_tree_BEFORE_UPDATE` BEFORE UPDATE ON `catalog_tree` FOR EACH ROW BEGIN
    ...
    ELSE
		IF ((old.parent_catalog_id IS NULL
			AND (SELECT COUNT(*) FROM catalog_tree
